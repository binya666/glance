...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($temp_image, $target_image)) {
            ...
            $MySqlCommand = "SELECT MAX(id) FROM teammember";
            ...
            $sql = "INSERT INTO teammember (id,name, designation, image, status) VALUES ($UserID, '$member_name', '$designation', '$image', '$Status')";
            if ($conn->query($sql) === TRUE) {
                header("Location: ../team-member-list.php");
                exit();
